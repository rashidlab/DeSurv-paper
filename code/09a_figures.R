#!/usr/bin/env Rscript
# code/09a_figures.R — Generate all figure objects from pre-computed results
#
# Rebuilds figure objects stored in results/ from model fits and validation
# results produced by earlier pipeline steps.
#
# The CV-based cutpoint analysis lives in code/08_cutpoint_analysis.R so
# that both this script and code/09b_si_figures.R can reuse the same
# (k, alpha)-conditional cutpoint selection without recomputing it.
#
# Inputs:  model fits, validation results from steps 04-05
#          cutpoint summaries / LP stats from step 08
# Outputs: figure objects in results/fig_*.rds
#
# Runtime: ~2 min (cutpoint CV is now in step 08)

message("=== Step 9a: Figures ===")
...
# ── NMF vs DeSurv Spearman correlation heatmaps ───────────────────────────
make_spearman_heatmap <- function(c_mat, row_labels, col_labels) {
  rownames(c_mat) <- row_labels
  colnames(c_mat) <- col_labels
  ph_args <- list(
    mat = c_mat,
...
fig_desurv_std_correlation_ntop <- make_spearman_heatmap(
  cor(fit_std_desurvk$W[unlist(tops_std), ], tar_fit_desurv$W[unlist(tops_de), ], method = "spearman"),
  heatmap_factor_labels_std, heatmap_factor_labels
)

tops_de  <- tar_tops_desurv$top_genes[1:50, ]
tops_std <- tar_tops_std_desurvk$top_genes[1:50, ]
fig_desurv_std_correlation_top50 <- make_spearman_heatmap(
  cor(fit_std_desurvk$W[unlist(tops_std), ], tar_fit_desurv$W[unlist(tops_de), ], method = "spearman"),
  heatmap_factor_labels_std, heatmap_factor_labels
)
...
# Cutpoint selection and LP standardisation stats are produced by step 08.
...
# Deduplicate PACA_AU (array + seq share subjects); mirrors 09b_si_figures.R
