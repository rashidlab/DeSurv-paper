...
![](figures/defense/fig4_forest.png){width="82%"}

::: {.notes}
The real test: does D1 transport? Five external cohorts (Dijk, Moffitt, both PACA-AU platforms, Puleo), scored purely by projection Z = W̃ᵀX_new — no retraining, no validation survival used in the fit. DeSurv's D1 (left) is protective and consistent: every cohort's CI sits at or below 1, pooled HR ≈ 0.76. NMF (right) has no factor with that clean, reproducible signal — the pooled estimates all straddle 1. Supervision didn't just reorganize the training factors; it produced a program that generalizes.
:::
...
::: {.notes}
Pooled validation KM at a cutpoint chosen by cross-validation on the training data. DeSurv's linear predictor splits high- vs low-risk cleanly: HR 1.79, log-rank p < 0.001, with a balanced split. Matched-rank NMF (click) can't: HR 1.38, p = 0.122, and it dumps almost everyone into the low-risk group (533 vs 37). Same rank, same projection rule — supervision is what turns the factors into a usable risk score. This survives adjustment for PurIST and DeCAF (HR 1.33, 95% CI 1.13–1.56), so it's independent prognostic value, not a restatement of existing subtypes.
:::
