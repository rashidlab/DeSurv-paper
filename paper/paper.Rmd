...
abstract: |
  Pancreatic ductal adenocarcinoma (PDAC) outcomes are shaped by the joint configuration of tumor-intrinsic and stromal programs, yet bulk-transcriptomic deconvolution methods that discover gene programs by variance maximization fragment this prognostic axis across multiple factors that fail to generalize across cohorts. We present DeSurv, a survival-supervised matrix factorization framework that jointly learns gene programs with a Cox proportional hazards model. Applied to pooled TCGA and CPTAC PDAC training data, DeSurv recovers a compact three-program decomposition centered on a single tumor--stroma coupling axis -- combining Classical tumor identity with restraining-CAF stroma -- that generalizes across five independent PDAC validation cohorts (Dijk, Moffitt, PACA-AU array and RNA-seq, Puleo; pooled hazard ratio per standard deviation = 1.50, 95% CI 1.31--1.72, P < 0.001) and retains independent prognostic value after adjustment for the established PurIST and DeCAF molecular classifiers, providing a single-sample-scorable risk stratifier for PDAC.
