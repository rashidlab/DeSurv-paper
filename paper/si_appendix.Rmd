...
In the metastatic RASH-ACCEPT cohort, $D_2$ was also associated with longer progression-free survival (HR per SD `r {r<-.tcs$pfs[.tcs$pfs$program=="D2",]; sprintf("$%.2f$ ($%.2f$--$%.2f$), %s", r$hr, r$lo, r$hi, .pfmt(r$p))}`); $D_3$ showed a directional but non-significant adverse association (`r {r<-.tcs$pfs[.tcs$pfs$program=="D3",]; sprintf("HR $%.2f$, %s", r$hr, .pfmt(r$p))}`). In the `r .tcs$paired$n` Linehan patients with paired pre- and post-treatment biopsies, $D_2$ increased significantly from pre- to post-treatment (paired Wilcoxon `r .pfmt(.tcs$paired$wilcox_p)`; mean $\Delta = `r sprintf("%+.2f", .tcs$paired$mean_delta)`$ SD), indicating that the proCAF program is itself remodelled by therapy.
