...
The primary analysis evaluated DeSurv in treatment-naive resected primary PDAC, the setting in which the gene-program matrix $W$ and Cox coefficients $\beta$ were trained. As an exploratory translational test we asked whether the same trained programs project sensibly into a clinically distinct cohort (microdissected metastatic specimens from patients receiving first-line chemotherapy), and specifically whether the $D_1$ protective effect on overall survival documented in the treatment-naive validation cohorts (main text Table~\ref{tab:hr-adjusted}, pooled adjusted HR $1.33$ per SD after PurIST + DeCAF adjustment, $P < 0.001$) persists in treated metastatic disease independently of the PurIST molecular classifier. We emphasize at the outset an important interpretive constraint: OKane biospecimens are laser-microdissected tumor, so the stromal compartment is physically depleted. The restCAF stromal genes that co-load on the $D_1$ program therefore have minimal expressional support in OKane, and the surviving $D_1$ signal reflects its Classical-epithelial (tumor-intrinsic) component rather than the coupled tumor--stroma axis that $D_1$ encodes in bulk tissue. The OKane finding below is consequently an \emph{epithelial-intrinsic} observation; it does not test, and should not be read as validating, the tumor--stroma coupling that is the manuscript's biological contribution. That coupling is established in the bulk treatment-naive validation cohorts (main text), where the stromal compartment is intact.
...
The factor-1 (D1) protective effect in OKane (HR $0.73$ per SD, $P < 0.001$) closely matches the D1 effect in the treatment-naive pooled validation (pooled HR $0.76$; main text Fig.\ \ref{fig:val}A), indicating that the epithelial component of the trained gene-program structure projects sensibly into a clinically distinct sample context (microdissected metastatic versus bulk primary, treated versus treatment-naive). The within-OKane pattern across factors is itself diagnostic of the stromal depletion: the epithelial-containing $D_1$ factor survives (HR $0.73$, $P < 0.001$), whereas the \emph{purely stromal} proCAF factor $D_2$ is null (HR $1.14$, $P = 0.16$) --- the same cohort and the same trained model, yet only the stromal factor collapses. This is the expected signature of microdissection: the proCAF genes that define $D_2$ have minimal expressional support once stroma is removed, while the Classical-tumor genes that anchor $D_1$ remain. Factor $D_3$ (Basal-like tumor) is also weak marginally. The marginal continuity of $D_1$'s effect (Figure~\ref{fig:figS-okane-marginal-continuity}) therefore provides face validity for the projection of the epithelial program specifically, not for the stromal axis.
...
\subsection{The epithelial $D_1$ signal in OKane is independent of PurIST}
\label{si:okane-purist}

We next tested whether $Z_1$'s protective association with overall survival in OKane is captured by the PurIST molecular classifier --- the established Basal-like/Classical tumor classifier for PDAC --- or whether $Z_1$ carries additional prognostic information independent of PurIST. After adjustment for PurIST in a Cox model fit on the same FFX+GA analytic sample ($n = 173$, $123$ events), $Z_1$ retains a significant protective effect: HR per SD $= 0.74$ ($95\%$ CI $0.59$--$0.92$, $P = 0.006$), nearly unchanged from the unadjusted marginal estimate (Table~\ref{tab:okane-marginal}, HR $0.73$, $P < 0.001$). The same pattern holds in both treatment arms separately: $Z_1$ PurIST-adjusted HR per SD $= 0.71$ ($95\%$ CI $0.53$--$0.96$, $P = 0.024$) in the FFX arm; $Z_1$ PurIST-adjusted HR per SD $= 0.73$ ($95\%$ CI $0.52$--$1.03$, $P = 0.069$) in the GA arm. The epithelial $D_1$ signal --- which in this microdissected cohort is its Classical-tumor component (Section~\ref{si:okane-marginal}) --- is therefore prognostic beyond the binary PurIST classification, indicating that the continuous $D_1$ program captures Classical-epithelial prognostic information not reducible to the Basal/Classical dichotomy. This is consistent with, but does not independently replicate, the main paper's PurIST + DeCAF-adjusted finding from treatment-naive primary PDAC (main text Table~\ref{tab:hr-adjusted}: pooled HR $1.33$ per SD, $P < 0.001$): DeCAF, the established stromal classifier, cannot be applied to OKane because its expressional substrate (intact stroma) is removed by microdissection, so the OKane result tests independence from the tumor-compartment classifier only and speaks to the epithelial axis alone.

\subsection{Caveats}

This analysis is exploratory and is limited to a single endpoint (overall survival) in a single cohort. (i) \emph{Microdissected substrate.} OKane is laser-microdissected tumor with the stromal compartment depleted, so the result speaks to the epithelial (Classical-tumor) component of $D_1$ and cannot test the tumor--stroma coupling that is the paper's biological contribution; that coupling is established in the bulk treatment-naive validation cohorts. (ii) $n = 173$, $123$ events; modest power for subgroup or interaction analyses. (iii) No analysis plan was preregistered. (iv) DeCAF was not applied because its expressional substrate (intact stroma) is removed by microdissection; the PurIST-adjustment claim is therefore one-sided (tumor classifier only) rather than the dual PurIST + DeCAF adjustment reported for treatment-naive validation in the main paper (Table~\ref{tab:hr-adjusted}). (v) Per-factor analyses for $Z_2$ and $Z_3$ are reported in Table~\ref{tab:okane-marginal} for transparency; both show weak or null marginal effects in OKane and do not support separate translational claims from this cohort. Prospective replication of the coupled program in an independent \emph{bulk-tissue} treated PDAC cohort with both PurIST and DeCAF available is the natural next test.
