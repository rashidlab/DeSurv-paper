...
The primary analysis evaluated DeSurv in treatment-naive resected primary PDAC, the setting in which the gene-program matrix $W$ and Cox coefficients $\beta$ were trained. As an exploratory translational test we asked whether the trained programs retain prognostic value in treated PDAC, and specifically whether they remain prognostic after adjustment for the established PurIST (tumor-intrinsic) and DeCAF (stromal) molecular classifiers --- the same dual adjustment applied to the treatment-naive validation cohorts in the main paper (Table~\ref{tab:hr-adjusted}). We used two independent bulk-tissue treated cohorts: RASH-ACCEPT ($n = 155$ metastatic PDAC, bulk RNA-seq, first-line chemotherapy) and Linehan ($n = 66$ borderline-resectable / locally advanced PDAC, bulk biopsy; FOLFIRINOX $\pm$ the CCR2 inhibitor PF-04136309). Both cohorts are bulk tissue, so PurIST and DeCAF are both computable and the full dual adjustment is valid --- unlike microdissected cohorts, where the stromal classifier cannot be applied.

\subsection{Per-factor and linear-predictor effects, unadjusted and PurIST + DeCAF-adjusted}

Factor scores were computed by projection using the same trained $W$ matrix as the main paper validation cohorts ($Z_i = X^\top W_{\cdot,i}$, full-W projection), z-standardized within cohort; the linear predictor (LP) is the trained-coefficient risk score $Z\beta$. Table~\ref{tab:rash-lin} reports per-cohort hazard ratios for overall survival per standard deviation, both marginal and after PurIST + DeCAF adjustment.

\begin{table}[H]
\centering
\caption{Overall-survival hazard ratios per SD for the DeSurv linear predictor (LP) and per-factor scores in two bulk treated PDAC cohorts, unadjusted and after PurIST + DeCAF adjustment. RASH-ACCEPT marginal $n = 155$ (130 events), adjusted $n = 125$ (107 events); Linehan $n = 66$ (22 events). \label{tab:rash-lin}}
\begin{tabular}{llcc}
\toprule
Cohort & Score & Marginal HR/SD (95\% CI), $P$ & PurIST + DeCAF-adjusted HR/SD (95\% CI), $P$ \\
\midrule
\multirow{4}{*}{RASH-ACCEPT}
 & LP        & $1.09$ ($0.93$--$1.30$), $0.29$  & $\mathbf{1.25}$ ($1.02$--$1.52$), $\mathbf{0.033}$ \\
 & $D_1$     & $0.93$ ($0.78$--$1.09$), $0.36$  & $\mathbf{0.81}$ ($0.66$--$0.99$), $\mathbf{0.038}$ \\
 & $D_2$     & $0.82$ ($0.68$--$0.99$), $0.039$ & $\mathbf{0.74}$ ($0.59$--$0.93$), $\mathbf{0.010}$ \\
 & $D_3$     & $1.08$ ($0.90$--$1.28$), $0.41$  & $0.99$ ($0.79$--$1.24$), $0.93$ \\
\midrule
\multirow{4}{*}{Linehan}
 & LP        & $\mathbf{1.82}$ ($1.10$--$3.03$), $\mathbf{0.021}$ & $1.23$ ($0.71$--$2.15$), $0.46$ \\
 & $D_1$     & $\mathbf{0.59}$ ($0.37$--$0.96$), $\mathbf{0.032}$ & $0.84$ ($0.49$--$1.43$), $0.52$ \\
 & $D_2$     & $\mathbf{0.64}$ ($0.44$--$0.94$), $\mathbf{0.022}$ & $\mathbf{0.51}$ ($0.33$--$0.78$), $\mathbf{0.002}$ \\
 & $D_3$     & $1.16$ ($0.73$--$1.83$), $0.53$  & $1.18$ ($0.67$--$2.08$), $0.56$ \\
\bottomrule
\end{tabular}
\end{table}

The two cohorts give complementary results. In RASH-ACCEPT, the LP and $D_1$ are not significant marginally but become independently prognostic after PurIST + DeCAF adjustment (LP HR $1.25$, $P = 0.033$; $D_1$ HR $0.81$, $P = 0.038$). This is a suppression pattern: the established classifiers are positively correlated with the DeSurv score and partially mask its prognostic effect, so the DeSurv signal is revealed only once they are conditioned out. The pattern is the bulk-tissue analogue of the main paper's finding that the DeSurv linear predictor retains prognostic value beyond PurIST + DeCAF (Table~\ref{tab:hr-adjusted}), and arguably strengthens the independence claim, since the classifiers do not merely fail to capture the DeSurv signal but oppose it. In Linehan, by contrast, $D_1$ and the LP are prognostic marginally ($D_1$ HR $0.59$, $P = 0.032$; LP HR $1.82$, $P = 0.021$) but their effect is absorbed by the joint adjustment, indicating that in that cohort the $D_1$ prognostic information is largely captured by PurIST + DeCAF. The one program independent of both classifiers in \emph{both} cohorts is the proCAF stromal program $D_2$ (RASH-ACCEPT HR $0.74$, $P = 0.010$; Linehan HR $0.51$, $P = 0.002$); we note its protective direction in treated disease runs opposite the aggressive proCAF association in treatment-naive primary tumors, and we do not interpret this further given the cohort heterogeneity.

\subsection{Caveats}

This analysis is exploratory. (i) The two cohorts give cohort-dependent results --- RASH-ACCEPT supports classifier-independent prognostic value of the LP and $D_1$, Linehan does not --- so the treated-cohort evidence is partial rather than uniform. (ii) The suppression pattern in RASH-ACCEPT (marginal null, adjusted significant) is statistically legitimate but should be presented as such rather than as a marginal prognostic claim. (iii) Cohorts differ in stage (RASH-ACCEPT metastatic; Linehan borderline-resectable / locally advanced), regimen (chemotherapy vs FOLFIRINOX $\pm$ CCR2 inhibitor), platform, and size (Linehan $n = 66$, 22 events, is small). (iv) No analysis plan was preregistered. (v) The $D_2$ direction reversal in treated disease relative to treatment-naive primaries is unexplained and not interpreted. Robust treated-cohort validation of the coupled tumor--stroma program will require larger, harmonized bulk-tissue series with both PurIST and DeCAF available.
