...
cross-sample variance fraction of factor $j$ was quantified as
$$V_j = \frac{\| (W_j H_j^\top)^{(c)} \|_F^2}{\| X^{(c)} \|_F^2},$$
where $W_j H_j^\top \in \mathbb{R}^{p \times n}$ is the rank-one outer product
contribution of factor $j$ to the reconstruction $WH$, the superscript $(c)$
denotes gene-by-gene centering across samples (subtracting each row's mean), and
$X^{(c)}$ is the centered expression matrix. This measures the cross-sample
variance encoded by factor $j$'s rank-one contribution, relative to the total
cross-sample variance in the data.

Gene centering is required to recover a metric that reflects cross-sample
variation. The uncentered analogue $\|W_j H_j^\top\|_F^2 / \|X\|_F^2$ is
dominated by the grand-mean baseline of the rank-normalized $X$ (sample ranks
$1, \ldots, p$ have no zeros, so $X$ has a large constant component) and
under-represents factors that encode genuine differences across samples.

For non-orthogonal factorizations such as NMF, the $V_j$ defined this way does
not partition to $\sum_j V_j = 1$ because the Frobenius cross-terms
$\langle (W_i H_i^\top)^{(c)}, (W_j H_j^\top)^{(c)} \rangle_F$ between
rank-one contributions are generally non-zero. In PDAC at $k = 3$, the centered
NMF per-factor fractions sum to approximately $1.97$, reflecting shared and
complementary structure between the three NMF factors. PCA partitions cleanly
because its components are orthogonal by construction. We report $V_j$ as the
isolated reconstruction contribution of each factor, recognizing that for
non-orthogonal factorizations such per-factor shares do not constitute a
partition of total variance.
...
\subsection{D1's protective effect on overall survival is independent of PurIST in OKane}
\label{si:okane-purist}

We next tested whether $Z_1$'s protective association with overall survival in OKane is captured by the PurIST molecular classifier --- the established Basal-like/Classical tumor classifier for PDAC --- or whether $Z_1$ carries additional prognostic information independent of PurIST. After adjustment for PurIST in a Cox model fit on the same FFX+GA analytic sample ($n = 173$, $123$ events), $Z_1$ retains a significant protective effect: HR per SD $= 0.74$ ($95\%$ CI $0.59$--$0.92$, $P = 0.006$), nearly unchanged from the unadjusted marginal estimate (Table~\ref{tab:okane-marginal}, HR $0.73$, $P < 0.001$). The same pattern holds in both treatment arms separately: $Z_1$ PurIST-adjusted HR per SD $= 0.71$ ($95\%$ CI $0.53$--$0.96$, $P = 0.024$) in the FFX arm; $Z_1$ PurIST-adjusted HR per SD $= 0.73$ ($95\%$ CI $0.52$--$1.03$, $P = 0.069$) in the GA arm. $Z_1$'s protective association with overall survival in OKane is therefore independent of PurIST, extending the main paper's PurIST + DeCAF-adjusted finding from treatment-naive primary PDAC (main text Table~\ref{tab:hr-adjusted}: pooled HR $1.33$ per SD, $P < 0.001$) to the treated metastatic setting on the OS endpoint. DeCAF, the established stromal classifier, was not applied to OKane samples in this analysis because OKane biospecimens are microdissected tumor with substantially depleted stromal content, the expressional substrate on which the DeCAF classifier was trained; the PurIST-adjusted result therefore tests independence from the tumor-compartment classifier only.

\subsection{Caveats}

This analysis is exploratory and is limited to a single endpoint (overall survival) in a single cohort. (i) $n = 173$, $123$ events; modest power for subgroup or interaction analyses. (ii) No analysis plan was preregistered. (iii) DeCAF was not applied owing to OKane's microdissected sample context; the PurIST-adjustment claim is therefore one-sided (tumor classifier only) rather than the dual PurIST + DeCAF adjustment reported for treatment-naive validation in the main paper (Table~\ref{tab:hr-adjusted}). (iv) Per-factor analyses for $Z_2$ and $Z_3$ are reported in Table~\ref{tab:okane-marginal} for transparency; both show weak or null marginal effects in OKane and do not support separate translational claims from this cohort. Prospective replication in an independent treated PDAC cohort with both PurIST and DeCAF available is the natural next test.
